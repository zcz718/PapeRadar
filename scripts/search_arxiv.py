...
Used by the start-my-week skill to find highly influential and high-quality
...
def title_to_note_filename(title: str) -> str:
    ...
    that wikilinks generated by start-my-week correctly resolve to files created
    by paper-analyze.
    """
...
def filter_and_score_papers(
    ...
    for paper in papers:
        ...
        if paper.get('source') in ('bioRxiv', 'medRxiv', 'PubMed') and matched_keywords:
...
def main():
    ...
    # Use shared resolver: $OBSIDIAN_VAULT_PATH/... → ~/.config/start-my-week/config.yaml
    try:
    ...
    parser.add_argument('--config', type=str,
                        ...
                        help='Path to research interests config file (defaults to ~/.config/start-my-week/config.yaml or $OBSIDIAN_VAULT_PATH/99_System/Config/research_interests.yaml)')
    ...
    # so start-my-week wikilinks can use this field directly without re-deriving it.
    ...
    output = {
        ...
        'bio_status': bio_status,  # "ok" | "import_failed:..." | "api_error:..."
        'total_unique': len(unique_papers),
        'top_papers': top_papers
    }

    # Save results — atomic write so concurrent /start-my-week runs (or
